...
def upgrade() -> None:
    op.create_table(
        ...
        sa.ForeignKeyConstraint(
            ["news_id"],
            ["news.id"],
            name=op.f("fk_news_acknowledgements_news_id_news"),
            ondelete="CASCADE",
        ),
        sa.ForeignKeyConstraint(
            ["user_eid"],
            ["employee.eid"],
            name=op.f("fk_news_acknowledgements_user_eid_employee"),
        ),
        sa.PrimaryKeyConstraint(
            "news_id", "user_eid", name=op.f("pk_news_acknowledgements")
        ),
    )
